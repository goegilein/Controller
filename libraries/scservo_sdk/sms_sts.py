...
class sms_sts(protocol_packet_handler):
    ...
    #addes dome more options
    def EnableTorque(self, scs_id: int, enable: int = 1):
        """enable: 1=ON, 0=OFF"""
        return self.write1ByteTxRx(scs_id, SMS_STS_TORQUE_ENABLE, 1 if enable else 0)

    def DisableTorque(self, scs_id: int):
        return self.write1ByteTxRx(scs_id, SMS_STS_TORQUE_ENABLE, 0)

    # ----- Mode helpers (servo vs. motor/wheel) -----
    def ServoMode(self, scs_id: int):
        """Position mode (absolute angle)."""
        return self.write1ByteTxRx(scs_id, SMS_STS_MODE, 0)

    def MotorMode(self, scs_id: int, closed_loop: bool = True):
        """Wheel / speed mode. closed_loop=True uses speed CL, False is open-loop."""
        return self.write1ByteTxRx(scs_id, SMS_STS_MODE, 1 if closed_loop else 2)
    
    # ----- Limits & calibration -----
    def SetAngleLimits(self, scs_id: int, min_pos: int, max_pos: int):
        """Set soft angle limits in position units (0..4095)."""
        tx = [self.scs_lobyte(min_pos), self.scs_hibyte(min_pos),
              self.scs_lobyte(max_pos), self.scs_hibyte(max_pos)]
        return self.writeTxRx(scs_id, SMS_STS_MIN_ANGLE_LIMIT_L, len(tx), tx)

    def SetOffset(self, scs_id: int, ofs: int):
        """Signed offset (-2048..+2047)."""
        tx = [self.scs_lobyte(ofs), self.scs_hibyte(ofs)]
        return self.writeTxRx(scs_id, SMS_STS_OFS_L, len(tx), tx)

    def SetDeadband(self, scs_id: int, cw: int, ccw: int):
        """Deadband in ADC ticks (0..255)."""
        if not (0 <= cw <= 255 and 0 <= ccw <= 255):
            return -1, -1  # or raise
        r1 = self.write1ByteTxRx(scs_id, SMS_STS_CW_DEAD, cw)
        r2 = self.write1ByteTxRx(scs_id, SMS_STS_CCW_DEAD, ccw)
        return r1 if r1[1]==0 else r1, r2

    # ----- Device / config -----
    def SetID(self, scs_id: int, new_id: int):
        return self.write1ByteTxRx(scs_id, SMS_STS_ID, new_id)

    def SetBaud(self, scs_id: int, baud_code: int):
        """
        Baud codes (per your file):
          0:1M, 1:500k, 2:250k, 3:128k, 4:115200, 5:76800, 6:57600, 7:38400
        """
        return self.write1ByteTxRx(scs_id, SMS_STS_BAUD_RATE, baud_code)
